var MEMBERS = [
  {
    name: "Angel A. García O'Diana",
    slug: "angel-a-garcia-odiana",
    photo: "Angel-4k.png",
    role: "Chief Executive Officer",
    area: "Dirección",
    bio: "Chief Executive Officer de PsiNet LAB. Lidera la visión estratégica de la organización, coordinando las diferentes áreas de investigación y proyectos sociales para asegurar el cumplimiento de los objetivos organizacionales.",
    interests: ["Psicología de la paz", "Ciencia de datos", "Métodos cuantitativos", "Metodología de la investigación", "Psicología budista"],
    email: ""
  },
  {
    name: "Álvaro J. Zumarán Baldeón",
    slug: "alvaro-j-zumaran-baldeon",
    photo: "alvaro-4k.jpg",
    role: "Chief Learning Officer",
    area: "Dirección",
    bio: "Alvaro Joseandree Zumarán Baldeón holds a Bachelor's degree in Psychology from the Pontifical Catholic University of Peru (PUCP) and is currently pursuing a second specialization in Clinical Psychoanalytic Intervention at the same university. He is an active member of PsiNet Lab, a research laboratory specializing in psychological research using advanced methodologies. At PsiNet Lab, Alvaro serves as both a researcher and Chief Learning Officer, contributing to the lab's research initiatives and educational efforts. His research interests include quantitative methods, psychotherapy process research, social cognition and the psychology of music. Through his work, he aims to bridge innovative methodologies with practical applications in psychological sciences.",
    interests: ["Quantitative methods", "Psychotherapy process research", "Social cognition", "Psychology of music"],
    email: ""
  },
  {
    name: "Fiorella G. Capcha Gonzales",
    slug: "fiorella-g-capcha-gonzales",
    photo: "fiorella-4k.jpg",
    role: "Chief Social Work Officer",
    area: "Dirección",
    bio: "Fiorella Grecia Capcha Gonzales holds a degree in Psychology from the Pontifical Catholic University of Peru (PUCP). She is currently pursuing a Second Specialization in Psychoanalytic Clinical Intervention at the same institution. She works as a researcher at PsiNet LAB (Peru), a center specialized in psychological research focused on the development and application of advanced methodologies. Her interests lie in integrating quantitative and qualitative approaches in areas related to social and community psychology, health, education, art, and technology. Through her work, she seeks to explore art and play as forms of individual expression, where connection is built beyond words.",
    interests: ["Social and community psychology", "Health", "Education", "Art and technology"],
    email: ""
  },
  {
    name: "Jefri A. Justo Mamani",
    slug: "jefri-a-justo-mamani",
    photo: "jefri-8k.jpeg",
    role: "Chief Communications Officer",
    area: "Dirección",
    bio: "Jefri Andy Justo Mamani is a Psychology student at the Universidad Nacional de San Agustín de Arequipa (UNSA), where he ranks 27th at the university level, demonstrating his commitment to academic excellence. He currently serves as Chief Communications Officer (CCO) at PsiNet LAB (Peru). Additionally, he has been a member of the Center for Research in Neuroscience and Neuropsychology (CEINPS), highlighting his interest in advancements in neuroscience. He stays updated and actively participates in various academic events, both as a participant and as part of the organizing team, in activities such as round tables, conferences, seminars, and debates at the Universidad Nacional de San Agustín de Arequipa (UNSA). His research interests focus on digital behavior, behavior in video games and social networks, as well as mental health and educational well-being.",
    interests: ["Digital behavior", "Behavior in video games and social networks", "Mental health", "Educational well-being"],
    email: ""
  },
  {
    name: "Valery Y. Marín Jaimes",
    slug: "valery-y-marin-jaimes",
    photo: "valery-4k.jpg",
    role: "Chief People Officer",
    area: "Dirección",
    bio: "Valery Yanira Marín Jaimes holds a Bachelor's degree in Psychology from the Pontifical Catholic University of Peru. As a member of PsiNet LAB (Peru), she actively contributes to research projects in positive psychology and acceptance and commitment therapy. Her research interests focus on applying quantitative methods to areas such as positive psychology, health and well-being, and education and technology, with a particular interest in learning and motivation within educational contexts. Valery is dedicated to advancing innovative methodologies that bridge psychology and technology, supporting progress in psychological sciences through her work at PsiNet LAB.",
    interests: ["Quantitative methods", "Positive psychology", "Health and well-being", "Education and technology", "Learning and motivation"],
    email: ""
  },
  {
    name: "Gianela K. Lucero Tarazona",
    slug: "gianela-k-lucero-tarazona",
    photo: "gianela-8k.jpg",
    role: "Chief Business Officer",
    area: "Dirección",
    bio: "Chief Business Officer de PsiNet LAB. Lidera la estrategia de negocios y el desarrollo organizacional, impulsando alianzas y oportunidades que fortalecen la sostenibilidad de la organización.",
    interests: ["Desarrollo organizacional", "Gestión estratégica", "Psicología empresarial"],
    email: ""
  },
  {
    name: "Luis E. Viloria Moreno",
    slug: "luis-e-viloria-moreno",
    photo: "luis-6k.jpeg",
    role: "Host del podcast \"La Ciencia de la Mente\"",
    area: "La Ciencia de la Mente",
    bio: "Host del podcast \"La Ciencia de la Mente\" en PsiNet LAB. Conduce conversaciones sobre temas de psicología y ciencia del comportamiento, acercando el conocimiento científico al público general.",
    interests: ["Divulgación científica", "Psicología", "Comunicación"],
    email: ""
  },
  {
    name: "Mavila L. Collanqui Narváez",
    slug: "mavila-l-collanqui-narvaez",
    photo: "mavila-hd.jpeg",
    role: "Miembro Investigadora",
    area: "Investigación",
    bio: "Mavila Lizbeth Collanqui Narvaez is a student of psychology at the National University of San Agustín of Arequipa (UNSA), consistently ranking among the top students in her program. Winner of first place in the scientific poster contest organized by the Catholic University of Santa María (UCSM), with the research titled \"Effect of Physical Attractiveness on the Perception of Street Sexual Harassment.\" Currently contributes to research involving vulnerable populations, exploring topics such as child development in low-resource contexts and street sexual harassment. Her research interests focus on applying scientific methodologies to analyze human behavior, particularly in organizational and workplace contexts.",
    interests: ["Scientific methodologies", "Human behavior analysis", "Organizational psychology", "Workplace contexts"],
    email: ""
  },
  {
    name: "Gleni R. Quispe Callo",
    slug: "gleni-r-quispe-callo",
    photo: "gleni-f.jpeg",
    role: "Miembro Investigadora",
    area: "Investigación",
    bio: "Gleni Quispe Callo holds a Bachelor's degree in Psychology from the Universidad Nacional de San Agustín de Arequipa (Peru), where she graduated with honors for her thesis on adolescent depression, demonstrating an early commitment to addressing mental health challenges during critical developmental stages. She furthered her education by obtaining a Master's degree in University Teaching from the Universidad San Martín de Porres, where her work expanded to include the study of anxiety and cognitive processes, focusing on how these psychological factors impact learning and decision-making in educational settings. Currently, she is a member of PsiNet LAB (Peru). She also holds a position as an Investigator at the Psychometrics Research Center at Universidad César Vallejo. Is a Renacyt researcher at Peru's National Council of Science and Technology (CONCYTEC). She also serves as an Editorial Assistant for INTERACCIONES: Journal of Family, Clinical and Health Psychology, contributing to the editorial process of high-impact publications in the fields of clinical psychology and public health. Additionally, she is an honorary member of the Centro de Estudios e Investigación en Neurociencias y Neuropsicología (CEINPS), reflecting her dedication to advancing knowledge in neuroscience. Her interdisciplinary interests are also evident through her membership in the Chemistry Student Society for Research-Peru. As a Research Assistant at the Neuroscience Laboratory of the Universidad Católica San Pablo. Her primary research interests include neuroscience and positive psychology, with a particular focus on anxiety, cognitive processes, decision-making, mild cognitive impairment, augmented reality, and EEG studies.",
    interests: ["Neuroscience", "Positive psychology", "Anxiety", "Cognitive processes", "Decision-making", "Mild cognitive impairment", "Augmented reality", "EEG studies"],
    email: ""
  },
  {
    name: "Marcelo S. Casa Chicata",
    slug: "marcelo-s-casa-chicata",
    photo: "marcelo-f.jpeg",
    role: "Chief Financial Officer",
    area: "Dirección",
    bio: "Marcelo Saúl Casa Chicata is a Psychology student at the Universidad Nacional de San Agustín (UNSA) in Arequipa, Peru. He currently serves as the CFO of PsiNet LAB (Peru) and works as a researcher, contributing to projects that utilize advanced methodologies such as network analysis. In addition, he is the director of the Center for Research in Neuroscience and Neuropsychology (CEINPS) at UNSA. His main research interests focus on quantitative methods and digital behavior, with additional emphasis on neuropsychology, social cognition, and interpersonal violence.",
    interests: ["Quantitative methods", "Digital behavior", "Neuropsychology", "Social cognition", "Interpersonal violence"],
    email: ""
  },
  {
    name: "Dennis S. Calle Huánuco",
    slug: "dennis-s-calle-huanuco",
    photo: "dennis-8k.jpg",
    role: "Miembro Investigador",
    area: "Investigación",
    bio: "Dennis Calle Huánuco holds a Bachelor's degree in Psychology, having graduated from César Vallejo University (UCV) with a thesis on individual differences in aversive personality traits and underlying emotions. He is currently completing a Master's degree in Higher Education at UCV, focusing on the heterogeneity and impact of higher educational characteristics and the sociology of education. He is a current member of PsinetLab, where he develops projects in psychology at both theoretical and practical levels, with a strong focus on quantitative methods and high-impact publications using innovative methodologies including network analysis. He is an officially active psychology researcher at the National Council of Science and Technology (CONCYTEC), with publications in clinical psychology, including studies on personality. He has served as a peer reviewer for the journal Interpersona. Additionally he has presented on the state of psychopathology in Latin America, having been invited by the International Society for the Study of Individual Differences, of which he is a member. His main research interests include positive psychology, educational differences, personality, aversive personality traits, nuances of violence, clinical comorbidities in psychology, and cross-cultural psychological research.",
    interests: ["Positive psychology", "Educational differences", "Personality", "Aversive personality traits", "Violence", "Clinical comorbidities", "Cross-cultural research"],
    email: ""
  },
  {
    name: "Katherine R. Olivera Rodrigo",
    slug: "katherine-r-olivera-rodrigo",
    photo: "kathy-4k.jpeg",
    role: "Miembro Investigadora",
    area: "Investigación",
    bio: "Katherine Rosa Olivera Rodrigo holds a Bachelor's degree in Nursing from Universidad Privada César Vallejo (Peru), where she ranked among the top five students in her class based on her cumulative GPA. She also completed a Master's degree in University Teaching at the same institution, earning the academic degree of Magister. Additionally, she is currently pursuing a second specialization in Mental Health and Psychiatry at Universidad Norbert Wiener in Peru. She has also completed updated courses in emergency and urgent care, basic life support, first aid, occupational health, and research methodology, among others. She is currently working as a registered nurse at a private educational institution, Organización Magister, where she provides preventive care and health services to children, adolescents, and teaching staff. In 2019, she received recognition for her outstanding job performance.",
    interests: ["Mental health", "Psychiatry", "University teaching", "Emergency care", "Research methodology"],
    email: ""
  },
  {
    name: "Andrea C. Moreno Tapia",
    slug: "andrea-c-moreno-tapia",
    photo: "andrea-sd.jpg",
    role: "Miembro Investigadora",
    area: "Investigación",
    bio: "Andrea Camila Moreno Tapia is a licensed psychologist from the Pontificia Universidad Católica del Perú (PUCP) and is currently pursuing a second specialization in Clinical Psychoanalytic Intervention at the same university. At PsiNet Lab, she works as a researcher with a particular interest in human identity processes, the psyche-soma relationship, and artistic and cultural manifestations. In addition to her background in psychology, she has a strong interest in performing arts and movement improvisation as potential psychotherapeutic tools. Through her research, she aims to contribute to their development.",
    interests: ["Human identity processes", "Psyche-soma relationship", "Artistic and cultural manifestations", "Performing arts", "Movement improvisation"],
    email: ""
  },
  {
    name: "Romell W. Poma Vivar",
    slug: "romell-w-poma-vivar",
    photo: "romell-sd.jpg",
    role: "Miembro Investigador",
    area: "Investigación",
    bio: "Economist with a strong academic background, including a Master's in Public Procurement Law and completed doctoral studies in Strategic Business Administration at CENTRUM PUCP (currently in the degree-granting process). He has over fifteen years of experience in both public and private sector entities. His specialization covers key areas of public management, such as Public Investment, Procurement, Strategic Planning, and Budgeting. Over the past two years, he has been conducting research aimed at generating empirical evidence on the impact of public policies, contributing to data-driven decision-making and the continuous improvement of government management. His experience in leading multidisciplinary teams has been strengthened through executive coaching programs, enabling him to successfully lead high-impact projects and efficiently manage human resources in the public sector.",
    interests: ["Public investment", "Procurement", "Strategic planning", "Budgeting", "Public policies"],
    email: ""
  },
  {
    name: "Marco A. Martínez Costilla",
    slug: "marco-a-martinez-costilla",
    photo: "Marco-sd.jpg",
    role: "Miembro Investigador",
    area: "Investigación",
    bio: "Miembro Investigador en PsiNet LAB. Contribuye al desarrollo de investigación psicológica innovadora, aplicando métodos científicos al estudio del comportamiento humano.",
    interests: ["Investigación psicológica", "Metodología científica", "Comportamiento humano"],
    email: ""
  },
  {
    name: "O. Carlos Puma Maque",
    slug: "o-carlos-puma-maque",
    photo: "carlos-f.jpeg",
    role: "Miembro Investigador",
    area: "Investigación",
    bio: "Carlos Puma Maque holds a bachelor's degree in Psychology from the National University of San Agustín de Arequipa (UNSA), where he is currently completing his graduation through the publication of his article titled Psychometric Properties of the Collective Moral Disengagement Scale in the Classroom among Peruvian Adolescents. Over the past year, he has produced several publications and is a researcher at Psinet Lab. He is currently pursuing his master's degree at UNSA, furthering his expertise and interest in quantitative and qualitative analysis, as well as in methodologies related to psychometrics and network analysis.",
    interests: ["Quantitative analysis", "Qualitative analysis", "Psychometrics", "Network analysis"],
    email: ""
  },
  {
    name: "Rudy M. Barrios Montealegre",
    slug: "rudy-m-barrios-montealegre",
    photo: "rudy-4k.jpeg",
    role: "Miembro Investigador",
    area: "Investigación",
    bio: "Miembro Investigador en PsiNet LAB. Participa en proyectos de investigación orientados al análisis y comprensión de fenómenos psicológicos y su aplicación práctica.",
    interests: ["Psicología aplicada", "Investigación científica", "Análisis del comportamiento"],
    email: ""
  },
  {
    name: "Hugo J. Suárez Horna",
    slug: "hugo-j-suarez-horna",
    photo: "hugo-sd.jpg",
    role: "Miembro Investigador",
    area: "Investigación",
    bio: "Miembro Investigador en PsiNet LAB. Trabaja en líneas de investigación que buscan profundizar el entendimiento del comportamiento humano desde una perspectiva científica.",
    interests: ["Comportamiento humano", "Psicología científica", "Investigación aplicada"],
    email: ""
  },
  {
    name: "Lillian A. Infante Damian",
    slug: "lillian-a-infante-damian",
    photo: "lillian-8k.jpg",
    role: "Miembro Investigadora",
    area: "Investigación",
    bio: "Miembro Investigadora en PsiNet LAB. Contribuye al desarrollo de proyectos de investigación psicológica orientados al bienestar y la comprensión del ser humano.",
    interests: ["Bienestar psicológico", "Investigación cualitativa", "Desarrollo humano"],
    email: ""
  },
  {
    name: "Fabrizio E. López de Pomar",
    slug: "fabrizio-e-lopez-de-pomar",
    photo: "fabrizio-4k.jpg",
    role: "Miembro Investigador",
    area: "Investigación",
    bio: "Miembro Investigador en PsiNet LAB. Participa en proyectos de investigación científica que buscan generar conocimiento innovador en el campo de la psicología.",
    interests: ["Psicología innovadora", "Investigación interdisciplinaria", "Ciencia del comportamiento"],
    email: ""
  }
];
